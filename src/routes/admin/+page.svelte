<script lang="ts">
	...
	import { addUsername, getAvailableUsernames, getUsers, removeUsername } from './admin.remote';
		...
		<svelte:boundary>
			{#each await getUsers() as user}
				<li class="flex items-center justify-between border-b p-2">
					...
					<!-- No delete button: removing a user needs a cascade across messages, keys,
					participations, owned groups and uploaded files, which is not implemented. -->
				</li>
			{/each}
			{#snippet pending()}
				<p>Loading users...</p>
