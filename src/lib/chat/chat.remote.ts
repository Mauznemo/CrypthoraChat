...
export function invalidateCachedUser(userId: string): void {
	...
	// The map is only ever written here and on a cache miss, so this is where stale entries get
	// swept - without it, it grows with every distinct user ever looked up.
	const now = Date.now();
