...
export async function tryGetPublicKey(): Promise<{ success: boolean; publicKey: string }> {
	try {
		...
		if (!isValid) {
			modalStore.open({
				...
				content:
					...
					'Either the master key on this device is wrong, something corrupted or someone tampered with the database.',
				buttons: [
					{
						text: 'Re-enter Master Key',
						variant: 'primary',
						onClick: () => {
							showMasterKeyImport();
						}
					...
					{
						text: 'Re-generate Public Key',
						variant: 'danger',
						onClick: () => {
							modalStore.confirm(
								'Are you sure?',
								'After re-generating your public key when you create a new chat or want to join one you will need to re-verify with the other users.',
								() => {
		...
		modalStore.error(e, 'Failed to get public key:');
		return { success: false, publicKey: '' };
	}
}
